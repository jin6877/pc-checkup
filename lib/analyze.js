...
function buildRamPlan({ memLayout, baseboard, totalGB, needUpgrade, recommendedGB, memoryLevel }) {
  ...
  const slots = baseboard && baseboard.memSlots ? baseboard.memSlots : 0;
  const populated = mods.length;
  const empty = slots > 0 ? Math.max(0, slots - populated) : 0;
  ...
  const soldered = mods.some(m => /LPDDR/.test(m.type) || /SOC|ROW/i.test(m.form)) || (slots === 0 && populated <= 1 && mods.every(m => /LPDDR/.test(m.type)));
  ...
  if (!needUpgrade || memoryLevel === 'low') {
    ...
    const after = totalGB + addSize * empty;
    action = `빈 슬롯 ${empty}개에 추가`;
    buy = `${spec || '기존과 동일 규격'} ${addSize}GB × ${empty}개 추가`;
    reason = `슬롯 ${slots}개 중 ${populated}개만 사용 중입니다. 지금 꽂힌 것과 같은 ${spec || '규격/속도'} ${addSize}GB를 ${empty}개 더 꽂으면 듀얼채널을 유지하면서 ${after}GB가 됩니다. 브랜드는 달라도 되지만 용량·속도는 맞추는 걸 권장.`;
  } else if (slots >= 2) {
    const perSlot = MODULE_TIERS.find(t => t >= Math.ceil(recommendedGB / slots)) || Math.ceil(recommendedGB / slots);
    action = '교체 (슬롯 가득 참)';
    buy = `${spec || '동일 규격'} ${perSlot}GB × ${slots}개 (키트로 구매)`;
    reason = `슬롯 ${slots}개가 이미 ${config}로 가득 찼습니다. 늘리려면 기존 걸 빼고 ${spec || '같은 규격'} ${perSlot}GB짜리 ${slots}개로 교체 → ${perSlot * slots}GB. 궁합 보장을 위해 낱개보다 ${slots}개 묶음(키트)을 사세요.`;
    if (maxGB && perSlot * slots > maxGB) reason += ` 단, 이 메인보드 최대 지원은 ${maxGB}GB이니 그 이하로 맞추세요.`;
  } else {
    action = '슬롯 정보 확인 필요';
    buy = `${spec || '기존과 동일 규격'}으로 증설/교체`;
    reason = `슬롯 수를 정확히 읽지 못했습니다. 케이스를 열어 빈 슬롯이 있으면 ${spec || '같은 규격'} 모듈을 추가하고, 없으면 더 큰 용량으로 교체하세요.`;
  }

  return { config, slots, populated, empty, spec, type, speed, maxGB, soldered, maker: mods[0] && mods[0].maker, action, buy, reason, buyable };
}
